...
import { FileSystemAdapter, MarkdownRenderer, MarkdownView, Notice, SettingTab } from 'obsidian';
...
async function getAppConfig(vaultBasePath: string): Promise<any> {
    return JSON.parse((await fs.promises.readFile(path.join(vaultBasePath, '.obsidian', 'appearance.json'))).toString());
}
...
async function getThemeCSS(settings: PandocPluginSettings, vaultBasePath: string): Promise<string> {
    ...
    try {
        ...
        if (settings.injectAppCSS === 'dark') light = false;
        let limitWidth = settings.limitHtmlBodyWidth;
        let maxWidth = settings.htmlBodyWidth;
        return appCSS(light, limitWidth, maxWidth);
    } catch (e) {
        return '';
    }
}
...
function getDesiredJavascript(settings: PandocPluginSettings, html: string, vaultBasePath: string){
    let javascript = `
function smoothScroll(href){
    var hashIndex = String(href).indexOf("#");
        if(hashIndex == -1)
            return;
        var target = href.substring(hashIndex + 1);
    document.querySelectorAll('[data-heading="' + target + '"]')[0].scrollIntoView({behavior: 'smooth'});
}`
...
async function standaloneHTML(settings: PandocPluginSettings, html: string, title: string, vaultBasePath: string): Promise<string> {
    ...
    const javascript = getDesiredJavascript(settings, html, vaultBasePath);
    ...
    outputFormat: string, parentFiles: string[] = [], css: string = '')
{
    ...
    // Fix a[class="internal-link"]
    // Fix <a href="app://obsidian.md/markdown_file_without_extension">
    const prefix = 'app://obsidian.md/';
    for (let a of Array.from(wrapper.querySelectorAll('a.internal-link')) as HTMLAnchorElement[]) {
        if (!a.href.startsWith(prefix)|| a.className!=='internal-link') 
            continue;
        ...
        if (settings.linkStrippingBehaviour === 'link' || outputFormat === 'html') {
            ...
            if (a.href.startsWith(prefix)) {
                ...
                if (path.extname(href) === '' && settings.addExtensionsToInternalLinks.length) {
                    ...
                    a.href = path.relative(dirname, href);
                }
                //Internal link to the same note
                else if (path.basename(href).startsWith('index.html')){
                    const dataHref = a.getAttribute('data-href');
                    const hashIndex = dataHref.indexOf('#');
                    if (hashIndex !== -1){
                        a.setAttribute('onclick', "smoothScroll(this.dataset['href'])");
                    }
                    a.href = 'javascript:void(0)';
                    a.removeAttribute('target');
                }
                //Other Internal link like pdf
                else
                    a.href = path.relative(dirname, href);
            }
            else 
    ...
    if (outputFormat !== 'html') {
        for (let img of Array.from(wrapper.querySelectorAll('img'))) {
            if (img.src.startsWith(prefix) && img.getAttribute('data-touched') !== 'true') {
                img.src = path.join(dirname, img.src.substring(prefix.length));
    ...
    //Remove copy buttons
    for (let botton of Array.from(wrapper.querySelectorAll('div.admonition-content-copy,button.copy-code-button'))){
        botton.parentElement.removeChild(botton);
    }
