...
export default class PandocPluginSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
        .setName("Limit html body width (HTML output only)")
        .setDesc("Limit maximum line length.")
        .addToggle(toggle => toggle
            .setValue(this.plugin.settings.limitHtmlBodyWidth)
            .onChange(async (value: boolean) => {
                this.plugin.settings.limitHtmlBodyWidth = value;
                this.display();
                await this.plugin.saveSettings();
            }));

        if(this.plugin.settings.limitHtmlBodyWidth)
            new Setting(containerEl)
            .setName("Maximum html body width")
            .addText(text => text
                .setPlaceholder('700px')
                .setValue(this.plugin.settings.htmlBodyWidth)
                .onChange(async (value: string) => {
                    if (!value.length) 
                        this.plugin.settings.htmlBodyWidth = '700px';
                    else 
                        this.plugin.settings.htmlBodyWidth = value;
                    await this.plugin.saveSettings();
                }));
